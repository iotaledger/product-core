...
fun test_role_based_permission_delegation() {
    ...
    // can't destroy the admin cap here because it's the last initial admin capability
...
#[test]
#[expected_failure(abort_code = role_map::ELastInitialAdminCapability)]
fun test_revoke_last_initial_admin_capability_fails() {
    let admin_user = @0xAD;
    let mut scenario = ts::begin(admin_user);
...
#[test]
#[expected_failure(abort_code = role_map::ELastInitialAdminCapability)]
fun test_destroy_last_initial_admin_capability_fails() {
    let admin_user = @0xAD;
    let mut scenario = ts::begin(admin_user);

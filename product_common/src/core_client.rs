...
pub trait CoreClientReadOnly {
    ...
    async fn get_object_by_id<T: DeserializeOwned>(
        &self,
        object_id: ObjectID,
    ) -> anyhow::Result<T> {
        self.client_adapter()
    ...
    async fn get_object_ref_by_id(
        &self,
        object_id: ObjectID,
    ) -> anyhow::Result<Option<OwnedObjectRef>> {
        self.client_adapter()
            .read_api()
    ...
    {
        let filter =
            IotaObjectResponseQuery::new_with_filter(IotaObjectDataFilter::StructType(tag));

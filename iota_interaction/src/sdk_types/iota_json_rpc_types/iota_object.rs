...
impl Display for IotaObjectData {
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    ...
    if let Some(storage_rebate) = self.storage_rebate {
      writeln!(writer, "{}: {storage_rebate}", "Storage Rebate",)?;
    }

    if let Some(previous_transaction) = self.previous_transaction {
      writeln!(writer, "{}: {previous_transaction:?}", "Previous Transaction",)?;
    }
  ...
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    ...
    match self {
      ...
      IotaParsedData::Package(p) => {
        write!(writer, "{}: {:?}", "Modules", p.disassembled.keys())?;
      }
    }
    write!(f, "{writer}")
  }
...
pub enum IotaObjectDataFilter {
  ...
  /// Query by type
  StructType(#[serde_as(as = "IotaStructTag")] StructTag),

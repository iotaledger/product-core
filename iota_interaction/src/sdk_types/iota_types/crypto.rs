  ...
  pub fn from_bytes(bytes: &[u8]) -> Result<Self, eyre::Report> {
    match SignatureScheme::from_flag_byte(bytes.first().ok_or_else(|| eyre!("Invalid length"))?) {
      Ok(x) => match x {
        ...
        SignatureScheme::Secp256k1 => Ok(IotaKeyPair::Secp256k1(Secp256k1KeyPair::from_bytes(
          bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
        )?)),
        SignatureScheme::Secp256r1 => Ok(IotaKeyPair::Secp256r1(Secp256r1KeyPair::from_bytes(
          bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
        )?)),
      ...
      Some(x) => {
        if x == &SignatureScheme::ED25519.flag() {
          let pk: Ed25519PublicKey = Ed25519PublicKey::from_bytes(
            bytes
              .get(1..)
              .ok_or(FastCryptoError::InputLengthWrong(Ed25519PublicKey::LENGTH + 1))?,
          )?;
          ...
          let pk = Secp256k1PublicKey::from_bytes(
            bytes
              .get(1..)
              .ok_or(FastCryptoError::InputLengthWrong(Secp256k1PublicKey::LENGTH + 1))?,
          )?;
          ...
          let pk = Secp256r1PublicKey::from_bytes(
            bytes
              .get(1..)
              .ok_or(FastCryptoError::InputLengthWrong(Secp256r1PublicKey::LENGTH + 1))?,
          )?;
...
impl PublicKey {
  ...
  pub fn try_from_bytes(curve: SignatureScheme, key_bytes: &[u8]) -> Result<PublicKey, Report> {
    match curve {
...
pub struct AuthorityPublicKeyBytes(#[serde_as(as = "Readable<Base64, Bytes>")] pub [u8; AuthorityPublicKey::LENGTH]);
...
pub trait IotaSignatureInner: Sized + ToFromBytes + PartialEq + Eq + Hash {
  ...
  fn get_verification_inputs(&self) -> IotaResult<(Self::Sig, Self::PubKey)> {
    ...
    let signature = Self::Sig::from_bytes(self.signature_bytes()).map_err(|_| IotaError::InvalidSignature {
  ...
  fn new(kp: &Self::KeyPair, message: &[u8]) -> Self {
    ...
    signature_bytes.extend_from_slice(&[<Self::PubKey as IotaPublicKey>::SIGNATURE_SCHEME.flag()]);
...
pub trait IotaSignature: Sized + ToFromBytes {
  ...
  fn verify_secure<T>(&self, value: &IntentMessage<T>, author: IotaAddress, scheme: SignatureScheme) -> IotaResult<()>
  where
    T: Serialize;
}
...
impl<S: IotaSignatureInner + Sized> IotaSignature for S {
    ...
    T: Serialize,
  {
    ...
    pk.verify(&digest, sig).map_err(|e| IotaError::InvalidSignature {

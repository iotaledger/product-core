...
impl MoveObjectType {
  ...
  pub fn is_coin_metadata(&self) -> bool {
    match &self.0 {
      MoveObjectType_::GasCoin | MoveObjectType_::StakedIota | MoveObjectType_::Coin(_) => false,
  ...
  pub fn is_coin_deny_cap_v1(&self) -> bool {
    self.address() == IOTA_FRAMEWORK_ADDRESS && self.module().as_str() == "coin" && self.name().as_str() == "DenyCapV1"
  }
...
impl fmt::Display for MoveObjectType {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    write!(f, "{}", to_iota_struct_tag_string(&s).map_err(fmt::Error::custom)?)
  }
}

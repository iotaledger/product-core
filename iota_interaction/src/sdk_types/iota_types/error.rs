...
pub enum UserInputError {
  ...
  AddressDeniedForCoin { address: IotaAddress, coin_type: String },
...
#[derive(Eq, PartialEq, Clone, Debug, Serialize, Deserialize, Hash, AsRefStr, IntoStaticStr, Error)]
  ...
  pub fn is_object_or_package_not_found(&self) -> bool {
    match self {
      IotaError::UserInput { error } => {
        matches!(
          error,
          UserInputError::ObjectNotFound { .. } | UserInputError::DependentPackageNotFound { .. }
        )
      }
      _ => false,
    }

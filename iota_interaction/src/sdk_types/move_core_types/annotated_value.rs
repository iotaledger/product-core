...
impl MoveTypeLayout {
  ...
  pub fn is_type(&self, type_: &TypeTag) -> bool {
    use {MoveTypeLayout as L, TypeTag as T};
  ...
  I: IntoIterator<Item = &'a MoveValue>,
{
  vals
    .into_iter()
    .map(|val| val.simple_serialize().expect("serialization should succeed"))
    .collect()
}
...
impl MoveVariant {
  ...
  pub fn undecorate(self) -> R::MoveVariant {
    R::MoveVariant {
      tag: self.tag,
      fields: self.into_fields().into_iter().map(MoveValue::undecorate).collect(),
    }
  }
}
...
impl<'d> serde::de::DeserializeSeed<'d> for &MoveEnumLayout {
  ...
  fn deserialize<D: serde::de::Deserializer<'d>>(self, deserializer: D) -> Result<Self::Value, D::Error> {
    let (variant_name, tag, fields) = deserializer.deserialize_tuple(2, DecoratedEnumFieldVisitor(&self.variants))?;
    Ok(MoveVariant {
      type_: self.type_.clone(),
      variant_name,
      tag,

...
#[derive(Subcommand)]
enum Commands {
    /// Create an initial Move.package-history.toml file from a Move.lock file
    Init {
        /// Path to the Move.lock file
        #[arg(short, long, default_value = "Move.lock")]
        move_lock: PathBuf,
        /// Output path for the Move.package-history.toml file
        #[arg(short, long, default_value = "Move.package-history.toml")]
        output: PathBuf,
    },
    /// Add a new package version to an existing Move.package-history.toml file
    Update {
        /// Path to the existing Move.package-history.toml file
        #[arg(short('f'), long, default_value = "Move.package-history.toml")]
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Update {
            ...
            let move_lock_path = move_lock.unwrap_or_else(|| {
                history_file
                    .parent()
                    .unwrap_or(Path::new("."))
                    .join("Move.lock")
...
/// Creates an initial Move.package-history.toml file from a Move.lock file
fn init_package_history(move_lock_path: &Path, output_path: &Path) -> Result<()> {
    ...
    let registry = PackageRegistry::from_move_lock_content(&move_lock_content)
        .context("Failed to parse Move.lock file")?;

    let toml_content = toml::to_string(&registry)
        .context("Failed to serialize PackageRegistry to TOML")?;

    fs::write(output_path, toml_content)
        .with_context(|| format!("Failed to write to output file: {}", output_path.display()))?;

    println!(
        "Successfully created Move.package-history.toml from {}",
...
/// Updates an existing Move.package-history.toml file with new package versions from a Move.lock file
fn update_package_history(history_file_path: &Path, move_lock_path: &Path) -> Result<()> {
    ...
    let history_content = fs::read_to_string(history_file_path).with_context(|| {
        format!(
            "Failed to read Move.package-history.toml file: {}",
            history_file_path.display()
        )
    })?;

    let mut registry = PackageRegistry::from_package_history_toml_str(&history_content)
        .context("Failed to parse existing Move.package-history.toml file")?;
    ...
    let new_registry = PackageRegistry::from_move_lock_content(&move_lock_content)
        .context("Failed to parse Move.lock file")?;
    ...
    let updated_toml_content = toml::to_string(&registry)
        .context("Failed to serialize updated PackageRegistry to TOML")?;

    fs::write(history_file_path, updated_toml_content).with_context(|| {
        format!(
...
fn create_backup_file(original_path: &Path) -> Result<()> {
    ...
    let backup_path = original_path.with_extension(format!("toml.bak-{}", timestamp));
...
"#.to_string()
    ...
    fn init_creates_package_history_from_move_lock() {
        ...
        let output_path = temp_dir.path().join("Move.package-history.toml");
    ...
    fn update_adds_new_package_versions() {
        let temp_dir = TempDir::new().unwrap();
        let history_path = temp_dir.path().join("Move.package-history.toml");
        let move_lock_path = temp_dir.path().join("Move.lock");
...
"#;
        ...
        let registry = PackageRegistry::from_package_history_toml_str(&updated_content).unwrap();
    ...
    fn update_creates_backup_file() {
        ...
        let backup_files: Vec<_> = fs::read_dir(temp_dir.path())
            ...
            .filter(|entry| {
                entry.file_name().to_string_lossy().starts_with("Move.package-history.toml.bak-")
            })
            .collect();
    ...
    fn update_fails_with_nonexistent_history_file() {
        let temp_dir = TempDir::new().unwrap();
        let history_path = temp_dir.path().join("nonexistent.toml");
        let move_lock_path = temp_dir.path().join("Move.lock");
